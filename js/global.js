...
//右边窗口商品数量按钮加减事件
function buyNumChangeBtn(){
	//右边窗口数量加减组件
	var oBtnPlus=$("button.glyphicon-plus-sign");
	var oBuyNums=$(".number");
	var oBtnMinus=$("button.glyphicon-minus-sign");
	for(var i=0;i<oBuyNums.length;i++){
		// 闭包解决事件获取正确索引i的问题
		(function(i){
			// 按钮+事件
           $(oBtnPlus[i]).click(function(){
           		 var num=$(oBuyNums[i]).val();
           	     if(num==""){
           	     	$(oBuyNums[i]).val(1);
           	     }
           	     else{
	           	 $(oBuyNums[i]).val(parseInt(num)+1);
	            }
           });
           //按钮-事件
           $(oBtnMinus[i]).click(function(){
	           	var num=$(oBuyNums[i]).val();
	           	console.log(num);
...
function AjaxPage(CurPage,PageSize){
    ...
    $.ajax({
        ...
        success:function(data){
            if(data==null){
            	$("div.item-dis").html("<span style='color:red;font-size:40px;'>未查询到数据！</span>");
            }
                ...
                if (data["Data"] != null) {  
                    ...
                    for (var i = 0; i < data["Data"].length; i++) {  
                        str += "<div class='bus-dis'></div>";//组合显示数据  
                    }  
                    $('.item-list').html(str);  
...
addLoadEvent(buyNumChangeBtn);
// addLoadEvent(btnSearch_Query);